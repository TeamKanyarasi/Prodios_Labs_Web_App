...
resource "aws_lb_target_group" "alb_tg" {
  ...
  target_type = "instance" # The default is "instance". (optional) "target type" you must specify when registering targets with this target group.
   ...
   health_check {
    ...
    interval            = 30 #(optional) Approximate amount of time, in seconds, between health checks of an individual target. The range is 5-300.
    path                = "/"
    timeout             = 10 # (optional) Amount of time, in seconds, during which no response from a target means a failed health check.
...
# Register 2 "be" Instances to ALB Target Group

resource "aws_alb_target_group_attachment" "alb_tg_add" {
